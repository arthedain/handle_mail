<template>
    <div>
        <div class="flex mb-6">
            ...
            <!--            <router-link :to="{name: 'handle-mail-single-resend', params: {id: mail.id}}" class="btn btn-default mr-2 btn-teal" style="margin-left: auto">-->
            <!--                {{__("Resend")}}-->
            <!--            </router-link>-->
        ...
        <card class="mb-6 py-3 px-6" v-if="!loading">
            <div v-for="(value, key)  in mail" :key="key" v-if="value && key !== 'ip_info'">
                <history v-if="key === 'history'" :data="value"></history>
                <div class="flex border-b border-40" v-if="key !== 'history'">
        ...
        <card class="mb-6 py-3 px-6" v-if="!loading">
            <div class="flex border-b border-40" v-for="(value, key)  in mail.ip_info" :key="key"
                 v-if="key !== 'driver' && value">
                <div class="w-1/4 py-4">
                    <h4 class="font-normal text-80">
                        {{ __(key) }}
                    </h4>
    ...
    export default {
        ...
        mounted() {
            axios.get('/nova-vendor/handle-mail/single/' + this.id,).then((response) => {
                ...
                // mail.created_at = moment(mail.created_at).locale('ru').format('LLLL');
                this.mail = mail;
                this.loading = false;
            });
        },
        methods: {
            ...
            //         .then((response) => {
            //             this.$toasted.show(this.__("Email resend successfully"), {
            //                 type: "success"
            //             });
            //         })
            //         .catch((error) => {
